(********************************************************************
  OKml – single‑file implementation for OneCompiler
  -------------------------------------------------
  This code satisfies the division‑of‑labor document but avoids any
  ocamllex/menhir syntax (which OneCompiler’s single‑file runner can’t
  compile).  Instead we provide:

  * `Exp`   – AST  (Person A)
  * `Lexer` – hand‑written tokenizer  (Person A)
  * `Parser`– recursive‑descent parser + precedence  (Person A)
  * `TC`    – monomorphic type checker (infer / unify)  (Person B)
  * `EV`    – evaluator + `constProp` optimisation         (Person C)
  * Demo / mini‑CLI + assertion tests
********************************************************************)
...
module Lexer = struct
  ...
  let of_string (src:string) : token list =
    ...
    in
    let rec eat_number i acc =
      if i<len && is_digit src.[i] then eat_number (i+1) acc else
      INT (int_of_string (String.sub src acc (i-acc))), i
    in
    let rec eat_ident i acc =
      if i<len && (is_letter src.[i] || is_digit src.[i]) then eat_ident (i+1) acc else
      let id = String.sub src acc (i-acc) in
      ...
      in tok, i
    in
    let two c1 c2 i = if i+1<len && src.[i]=c1 && src.[i+1]=c2 then true else false in
    let rec go i acc =
      let i = skip_ws i in
      if i>=len then List.rev (EOF::acc) else
      let c = src.[i] in
      if is_digit c then
        let tok, j = eat_number (i+1) i in go j (tok::acc)
      else if is_letter c then
        let tok, j = eat_ident (i+1) i in go j (tok::acc)
      else match c with
        | '='                     -> go (i+1) (EQ::acc)  (* <-- add this *)
      | '!' when two '!' '=' i -> go (i+2) (NEQ::acc)
      | '<' when two '<' '=' i -> go (i+2) (LE::acc)
      | '>' when two '>' '=' i -> go (i+2) (GE::acc)
      | '&' when two '&' '&' i -> go (i+2) (AND::acc)
      | '|' when two '|' '|' i -> go (i+2) (OR::acc)
      | '-' when two '-' '>' i -> go (i+2) (ARROW::acc)
      | '+' -> go (i+1) (PLUS::acc)
      | '-' -> go (i+1) (MINUS::acc)
      | '*' -> go (i+1) (TIMES::acc)
      | '/' -> go (i+1) (DIV::acc)
      ...
      | '(' -> go (i+1) (LP::acc)
      | ')' -> go (i+1) (RP::acc)
      | _   -> failwith (sprintf "Lexer: unknown char '%c'" c)
    in
    go 0 []
end
...
module Parser = struct
  ...
  and parse_let st =
    ...
    | LET ->
        ignore (eat st);           (* let *)
        let x = (match eat st with IDENT id -> id | _ -> raise (Parse_error "id")) in
        expect st EQ;              (* =  *)
        let e1 = expr st in
        expect st IN;              (* in *)
        let e2 = expr st in
  ...
  and parse_or st =
    ...
    let rec loop acc =
      match peek st with OR -> ignore(eat st); let rhs = parse_and st in loop (Binop(Or,acc,rhs)) | _ -> acc
    in loop lhs
  and parse_and st =
    let lhs = parse_eq st in
    let rec loop acc =
      match peek st with AND -> ignore(eat st); let rhs = parse_eq st in loop (Binop(And,acc,rhs)) | _ -> acc
    in loop lhs
  and parse_eq st =
  ...
  and parse_app st =
    let rec collect acc =
      match peek st with INT _ | BOOL _ | IDENT _ | LP | IF | FUN ->
        collect (App(acc, parse_atom st))
      | _ -> acc
    in collect (parse_atom st)
  and parse_atom st =
    match eat st with
    | INT n   -> Int n
    | BOOL b  -> Bool b
    ...
    | _ -> raise (Parse_error "atom")

  let parse_string s =
    let st = of_tokens (Lexer.of_string s) in
    let res = expr st in
    (match peek st with EOF -> () | _ -> raise(Parse_error "trailing input"));
    res
end
...
module TC = struct
  ...
  let rec infer env = function
    ...
    | Let(x,e1,e2) -> let t1 = infer env e1 in infer (M.add x t1 env) e2
    | Fun(x,b)     -> let tv = TInt in TFun(tv, infer (M.add x tv env) b)
    | App(f,a)     ->
        let tf = infer env f in
        let ta = infer env a in
        (match tf with TFun(p,r)->unify p ta; r | _ -> raise (Type_error "Expected function"))
    | If(c,t,e)    -> unify (infer env c) TBool; let tt = infer env t in unify tt (infer env e); tt
    | Binop(op,l,r)->
        (match op with
         | Add|Sub|Mul|Div -> unify (infer env l) TInt; unify (infer env r) TInt; TInt
         | Eq|Neq|Lt|Gt|Le|Ge -> let tl=infer env l in unify tl (infer env r); TBool
         | And|Or -> unify (infer env l) TBool; unify (infer env r) TBool; TBool)
end
...
module EV = struct
  ...
  let rec eval env = function
    ...
    | Let(x,e1,e2)-> let v1 = eval env e1 in eval ((x,v1)::env) e2
    | Fun(x,b)    -> Fun(x,b)
    | App(f,a)    -> let vf = eval env f in let va = eval env a in (match vf with Fun(x,b)->eval ((x,va)::env) b |_ -> raise(Runtime_error"Expected function"))
    | If(c,t,e)   -> (match eval env c with Bool true->eval env t | Bool false->eval env e |_ -> raise(Runtime_error"Expected bool"))
    | Binop(op,l,r) ->
        let v1 = eval env l in let v2 = eval env r in
        match op,v1,v2 with
  ...
  let rec constProp = function
    | Binop(op,Int a,Int b) -> (match op with Add->Int(a+b)|Sub->Int(a-b)|Mul->Int(a*b)|Div when b<>0->Int(a/b)|_->Binop(op,Int a,Int b))
...
let () =
  ...
  let run src =
    let ast  = Parser.parse_string src in
    ignore (TC.infer TC.M.empty ast);          (* type‑check *)
    let ast' = EV.constProp ast in             (* optimise   *)
  ...
  assert (run "2 + 3 * 4"                          = "14");   (* precedence          *)
  assert (run "3 < 5"                              = "true"); (* comparison          *)
  assert (run "true && false || true"              = "true"); (* short‑circuit && || *)
  assert (run "let x = 2 in let x = 3 in x"        = "3");    (* nested let/shadow   *)
  assert (run "let inc = (fun y -> y + 1) in inc 7"= "8");    (* function value      *)
  assert (run "let x = 3 + 4 in x * 2"             = "14");   (* const‑prop folding  *)
  assert (run "(fun f -> f 4) (fun z -> z * z)"    = "16");   (* higher‑order func   *)
...
(*1st draft Searching funtionc, checks, test cases A I*)